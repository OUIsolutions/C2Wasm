...
EM_JS(c2wasm_js_var,async_c2wasm_call_object_prop,(c2wasm_js_var stack_index, const char *prop_name,c2wasm_js_var args),{
    let object = window.c2wasm_stack[stack_index];
    let prop_name_formatted = window.c2wasm_get_string(prop_name);
    let arguments = undefined;
    if(args != -1){
    ...
    (async function(){
        try{
            result = await object[prop_name_formatted](...arguments);
        }catch(error){
            result = error;
        }     
    })();
